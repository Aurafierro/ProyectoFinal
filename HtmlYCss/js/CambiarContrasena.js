document.addEventListener('DOMContentLoaded', function () {
    ...
    async function changePassword(nuevaContrasena, confirmarContrasena) {
        ...
        try {
            const response = await fetch('http://10.192.92.90:8080/api/v1/user/cambiar-contrasena', {
                method: 'PUT',
                headers: {
    ...
    async function redirectAfterPasswordChange(token) {
        try {
            const response = await fetch('http://10.192.92.90:8080/api/v1/user/rol', {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`, // Aquí se corrigió
                    'Content-Type': 'application/json'
            ...
            if (userRole === "Administrador") {
                window.location.href = 'http://10.192.92.90:5500/HtmlYCss/indexHTML/M.informacionAdmin.html';
                ...
                window.location.href = 'http://10.192.92.90:5500/HtmlYCss/indexHTML/InicioSesion.html'; // Ajusta la URL según sea necesario
            }
